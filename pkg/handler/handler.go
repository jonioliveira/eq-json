...
func FilesAreEqual(file1, file2 []byte) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("Error mashalling file 1: %s", err.Error())
	}
	err = json.Unmarshal(file2, &result2)
	if err != nil {
		return false, fmt.Errorf("Error mashalling file 2: %s", err.Error())
	}
...
func ReadFiles(paths []string) (map[string][]byte, error) {
	...
	for _, path := range paths {
		...
		if !json.Valid(byteValue) {
			return filesContentMap, fmt.Errorf("Invalid json file with path %s", path)
		}
